  ...
  try {
    ...
    res.render("course", {
      ...
      course: {
        ...
        type: fmtCourseType(course.type),
...
export const bookingConfirmationPage = async (req, res, next) => {
  try {
    ...
    if (booking.userId !== req.user._id && req.user.role !== "organiser") {
    ...
    const rows = await Promise.all(
      bookings.map(async (b) => {
        ...
        if (b.type === "COURSE") {
          ...
          if (course) {
            title = course.title;
            when = course.startDate ? fmtDateOnly(course.startDate) : "";
          }
        }
        ...
        if (b.type === "SESSION") {
          ...
          if (session) {
            ...
            title = course ? `${course.title} (single session)` : "Single session";
            when = session.startDateTime ? fmtDate(session.startDateTime) : "";
          }
        }
